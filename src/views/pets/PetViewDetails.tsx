...
import { Fingerprint, Info, PawPrint, Rainbow, Weight } from "lucide-react";

export const PetViewDetails = ({
  pet,
  onClose,
}: {
  pet: PetSchema;
  ...
  return (
    <FullScreenModal
      isOpen={true}
      title="Informacion Mascota"
      onClose={onClose}
      isBack={false}
    >
      <div className="flex flex-col  gap-10 md:flex-row">
        <div className=" flex items-center mt-16   w-full flex-col md:w-1/2">
          {pet.imagePet ? (
            <img
              src={pet?.imagePet}
              alt=""
              className=" size-32 border-2 border-gray-400 rounded-full"
            />
          ) : (
        ...
        <div className="w-full">
          <div className="mt-5 flex items-center bg-slate-100 w-full gap-4 rounded-md p-2 dark:bg-slate-600 dark:text-slate-50">
            ...
            <p className="text-md font-semibold  w-full capitalize">
              <span className=" font-normal text-sm block text-gray-600 dark:text-slate-50 ">
                Raza-Especie
              </span>
              {pet.especie}
              {" - "}
          ...
          <div className="mt-5 flex  bg-slate-100 w-full gap-4 rounded-md p-2 dark:bg-slate-600 dark:text-slate-50">
